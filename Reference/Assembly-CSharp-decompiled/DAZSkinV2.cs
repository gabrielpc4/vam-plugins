...
[ExecuteInEditMode]
public class DAZSkinV2 : PreCalcMeshProvider
{
	...
	public Vector3[] RecalcAndGetAllNormals(Vector3[] vertsToUse)
	{
		...
		foreach (DAZVertexMap dAZVertexMap in array)
		{
			ref Vector3 reference = ref postSkinWorkingNormals[dAZVertexMap.tovert];
			reference = postSkinWorkingNormals[dAZVertexMap.fromvert];
		}
	...
	public void SmoothAllVertices(Vector3[] inVerts, Vector3[] outVerts)
	{
		...
		foreach (DAZVertexMap dAZVertexMap in array)
		{
			ref Vector3 reference = ref outVerts[dAZVertexMap.tovert];
			reference = outVerts[dAZVertexMap.fromvert];
		}
	}
	...
	public void SkinMeshThreadedFast(Vector3[] verts, bool forceSynchronous = false)
	{
		...
		{
			ref Vector3 reference = ref workingVerts[dAZVertexMap.tovert];
			reference = workingVerts[dAZVertexMap.fromvert];
		}
	...
	protected void SkinMeshStartFrame(bool forceSynchronous = false)
	{
		if (useAsynchronousThreadedSkinning && !forceSynchronous && skinMethod != SkinMethod.GPU && dazMesh is DAZMergedMesh)
		{
			...
			dAZMergedMesh.UpdateVerticesPre(forceChange: true);
			if (_useGeneralWeights)
			{
		...
		else
		{
			...
			if (_useGeneralWeights)
			{
				...
				{
					ref Vector3 reference2 = ref startVertsCopy[j];
			...
			else
			{
				...
				{
					ref Vector3 reference3 = ref workingVerts[k];
					reference3 = startVerts[k];
				}
			}
		}
		postSkinVertsChangedThreaded = postSkinVertsChanged;
	...
	protected void SkinMeshThreaded(bool forceSynchronous = false)
	{
		...
		if (allowPostSkinMorph)
		{
			...
			else
			{
				for (int m = 0; m < postSkinNeededVertsList.Length; m++)
				{
					...
					ref Vector3 reference2 = ref rawSkinnedWorkingVerts[num4];
					reference2 = workingVerts[num4];
				}
			}
		}
		if (needsPostSkinNormals)
		...
		if (_useSmoothing && skinMethod == SkinMethod.CPU)
		{
			...
			if (useMultithreadedSmoothing && !forceSynchronous && _numSubThreads > 0)
			{
				...
				foreach (DAZVertexMap dAZVertexMap in baseVerticesToUVVertices)
				{
					ref Vector3 reference3 = ref smoothedVerts[dAZVertexMap.tovert];
					reference3 = smoothedVerts[dAZVertexMap.fromvert];
				}
			}
			else
			{
				...
				{
					ref Vector3 reference4 = ref smoothedVerts[dAZVertexMap2.tovert];
					reference4 = smoothedVerts[dAZVertexMap2.fromvert];
				}
			}
		}
		else
		{
			...
			{
				ref Vector3 reference5 = ref workingVerts[dAZVertexMap3.tovert];
				reference5 = workingVerts[dAZVertexMap3.fromvert];
			}
		}
		threadMainSkinStopTime = (float)stopwatch.ElapsedTicks * f;
		threadMainSkinTime = threadMainSkinStopTime - threadMainSkinStartTime;
		...
		else
		{
			...
			for (int i = 0; i < mesh.subMeshCount; i++)
			{
				if (dazMesh.materialsEnabled[i])
				{
					...
					{
						Graphics.DrawMesh(mesh, identity, dazMesh.simpleMaterial, base.gameObject.layer, null, i, null, materialsShadowCastEnabled[i], dazMesh.receiveShadows);
					}
					else if (dazMesh.materials[i] != null)
					{
						Graphics.DrawMesh(mesh, identity, dazMesh.materials[i], base.gameObject.layer, null, i, null, materialsShadowCastEnabled[i], dazMesh.receiveShadows);
					}
				}
			}
		}
	...
	protected void DrawMeshNative()
	{
		...
		for (int i = 0; i < mesh.subMeshCount; i++)
		{
			if (dazMesh.materialsEnabled[i])
			{
				...
				{
					component2.materials[i] = dazMesh.simpleMaterial;
				}
				else
				{
