...
/*					if (currentAtom.type == "Person")
        ...
        public void FixedUpdate()
        {
			...
			if (uiDoMorphs.val)
			{
				...
				if (false && morphDeepBulgeBellyBottom != null && morphDeepBulgeBellyMid != null && usePerson2 && uiDoSex.val)
				{
				...
				if (false && morphDeepThroat != null && usePerson2 && uiDoBlowjob.val)
				{
				...
				if (false && morphBlowjobLips != null && morphCheekSink != null && usePerson2 && playerTipToHead < playerHeadToHead && playerTipToHead < personalSpaceDistance/2.0f)
				{
				...
				if (frames % 1 == 0)
			{
				...
				if (false && mTongueInOutTarget > mTongueInOutValue + 0.01f) { mTongueInOutValue = Mathf.Min(mTongueInOutValue + (0.08f * mouthVariation * morphSpeed), mTongueInOutTarget); }
				if (false && mTongueInOutTarget < mTongueInOutValue - 0.01f) { mTongueInOutValue = Mathf.Max(mTongueInOutValue - (0.05f * mouthVariation * morphSpeed), mTongueInOutTarget); }
				morphTongueInOut.SetValue(Mathf.Round(mTongueInOutValue * 10.0f) / 10.0f);
				if (false && mTongueSideSideTarget > mTongueSideSideValue + 0.01f) { mTongueSideSideValue = Mathf.Min(mTongueSideSideValue + (0.084f * mouthVariation * morphSpeed), mTongueSideSideTarget); }
				if (false && mTongueSideSideTarget < mTongueSideSideValue - 0.01f) { mTongueSideSideValue = Mathf.Max(mTongueSideSideValue - (0.017f * mouthVariation * morphSpeed), mTongueSideSideTarget); }
				morphTongueSideSide.SetValue(Round(mTongueSideSideValue));
				if (false && mTongueBendTipTarget > mTongueBendTipValue + 0.01f) { mTongueBendTipValue = Mathf.Min(mTongueBendTipValue + (0.08f * mouthVariation * morphSpeed), mTongueBendTipTarget); }
				if (false && mTongueBendTipTarget < mTongueBendTipValue - 0.01f) { mTongueBendTipValue = Mathf.Max(mTongueBendTipValue - (0.06f * mouthVariation * morphSpeed), mTongueBendTipTarget); }
...
//					lookAtPosition = playerLHandTransform.TransformPoint(new Vector3(0.2f, 0.0f, 0.2f));
            ...
            //if (1.0f == 1.0f)
			{
                ...
                if ((secondInterest == "LHand" && interestLHand + lHandActivityBoost > 20.0f) || (secondInterest == "RandomL" && interestLHand + lHandActivityBoost > 10.0f))
                {
                ...
                if ((secondInterest == "RHand" && interestRHand + rHandActivityBoost > 20.0f) || (secondInterest == "RandomR" && interestRHand + rHandActivityBoost > 10.0f))
                {
