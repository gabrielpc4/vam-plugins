...
VaM Gabriel workflows treat ``Default.json`` as the edited hub scene, then mirror
to ``MainMenu.json`` so both stay byte-for-byte the same.

Run from VaM root:
  python Custom/Scripts/Gabriel/tools/scene-menu/sync_hub_default_mainmenu.py
