...
namespace VRAdultFun
{
    partial class EmotionEngine
    {
        private class BConcentrate : State
        {
            ...
            {
				Duration = 0.001f;
				if (enableConcentrate && (interestArousal < 4.0f || interestValence > 7.0f))
				{
				...
				//SuperController.LogMessage("Full Face set to 0 for concentrate brow", false);
                mSmileOpenFullFaceTarget = 0.0f;
                mMouthOpenTarget = 0.0f;
                //mLipsPuckerTarget = 0.0f;
                mHappyTarget = 0.0f;
                Duration = Random.Range(1.15f, 7.35f) * lookVariation;
				}
            }
