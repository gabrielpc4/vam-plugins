...
public class AdjustJointTarget : MonoBehaviour
{
	...
	private void SetTargetRotationFromPercent()
	{
		if (_xPercent < 0f)
		{
			_currentTargetRotation.x = Mathf.LerpUnclamped(_zeroTargetRotation.x, _lowTargetRotation.x, 0f - _xPercent);
		}
		else
		{
			_currentTargetRotation.x = Mathf.LerpUnclamped(_zeroTargetRotation.x, _highTargetRotation.x, _xPercent);
		}
		if (_yPercent < 0f)
		{
			_currentTargetRotation.y = Mathf.LerpUnclamped(_zeroTargetRotation.y, _lowTargetRotation.y, 0f - _yPercent);
		}
		else
		{
			_currentTargetRotation.y = Mathf.LerpUnclamped(_zeroTargetRotation.y, _highTargetRotation.y, _yPercent);
		}
		if (_zPercent < 0f)
		{
			_currentTargetRotation.z = Mathf.LerpUnclamped(_zeroTargetRotation.z, _lowTargetRotation.z, 0f - _zPercent);
		}
		else
		{
			_currentTargetRotation.z = Mathf.LerpUnclamped(_zeroTargetRotation.z, _highTargetRotation.z, _zPercent);
		}
	}
