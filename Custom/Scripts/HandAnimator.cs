		...
		void Update() {
			try {
				...
				if (uianimatehands.val)
				{
					...
					if ((lchestDist > minChestDist && labsDist > minAbsDist && lheadDist > minHeadDist && lhipLDist > minHipDist && lhipRDist > minHipDist && lnippleLDist > minNippleDist && lnippleRDist > minNippleDist) || lmoveDist > minMotion)
					{
						if (lcurhandpos.y > neckController.followWhenOff.position.y)
						{
							if (lcurhandpos.y < headController.followWhenOff.position.y + 0.15f && lheadDist < 0.55f * uidistancescale.val)
							{
								...
								mLHandFistTarget = maxFist * (Mathf.Clamp(lmoveDist*33.0f, 0.0f, 1.0f));
								lhanddebug += " ParHead";
							}
							else
							{
					...
					if ((rchestDist > minChestDist && rabsDist > minAbsDist && rheadDist > minHeadDist && rhipLDist > minHipDist && rhipRDist > minHipDist && rnippleLDist > minNippleDist && rnippleRDist > minNippleDist) || rmoveDist > minMotion)
					{
						if (rcurhandpos.y > neckController.followWhenOff.position.y)
						{
							if (rcurhandpos.y < headController.followWhenOff.position.y + 0.15f && rheadDist < 0.55f * uidistancescale.val)
							{
								...
								mRHandFistTarget = maxFist * (Mathf.Clamp(rmoveDist*33.0f, 0.0f, 1.0f));
								rhanddebug += " ParHead";
							}
							else
							{
					...
					morphLThumbGrasp.SetValue((Round(mLHandFistValue - mLHandStraightenValue) * uithumbadjust.val) + uithumboffset.val);
					morphRThumbGrasp.SetValue((Round(mRHandFistValue - mRHandStraightenValue) * uithumbadjust.val) + uithumboffset.val);
					morphLThumbFist.SetValue((Round(mLHandStraightenValue + mLHandFistValue) * uithumbadjust.val) + uithumboffset.val);//Round(mLHandFistValue - (mLHandStraightenValue / 2.0f) + (Mathf.Max(-mLHandChopValue, 0.0f)), Mathf.Lerp(-0.5f, 0.0f, Round(lmoveDist*100.0f, 0.0f, 1.0f)), 1.0f));
					morphRThumbFist.SetValue((Round(mRHandStraightenValue + mRHandFistValue) * uithumbadjust.val) + uithumboffset.val);//Round(mRHandFistValue - (mRHandStraightenValue / 2.0f) + (Mathf.Max(-mRHandChopValue, 0.0f)), Mathf.Lerp(-0.5f, 0.0f, Round(rmoveDist*100.0f, 0.0f, 1.0f)), 1.0f));
