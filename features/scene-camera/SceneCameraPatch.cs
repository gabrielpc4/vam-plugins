...
namespace geesp0t
{
    ...
    {
        public const string PatchScriptRelative = "Custom/Scripts/tools/scene-camera/patch_scene_initial_camera.py";

        /// <summary>Written on every successful K diagnostics pass (automatic or manual follow-up).</summary>
        public const string RequestJsonRelative = "Custom/Scripts/tools/scene-camera/last_scene_camera_patch_request.json";

        /// <summary>
        /// Optional manual wrapper with friendlier CLI help (same argv as <see cref="PatchScriptRelative"/>).
        /// </summary>
        public const string ManualCliScriptRelative = "Custom/Scripts/tools/scene-camera/patch_scene_camera_manual_cli.py";

        /// <summary>
        /// Append-only log next to <see cref="PatchScriptRelative"/>.
        /// </summary>
        public const string PatchToolLogRelative = "Custom/Scripts/tools/scene-camera/last_scene_camera_patch_log.txt";
        ...
        public static void TryRunFromHotkey()
        {
            ...
            {
                SuperController.LogError("Hud [key K]: SuperController.singleton is null.");
                return;
            }
            ...
            {
                SuperController.LogError("Hud [key K]: SuperController.currentLoadDir is empty; cannot infer scene folder.");
                return;
            }
            ...
            if (jsonBasenamesSortedDistinct == null)
            {
                SuperController.LogError(string.Format(
                    "Hud [key K]: could not enumerate .json scene files under {0}.",
                    loadDirectoryFwdTrimmedNormalized));
                return;
            }
            ...
            catch (Exception requestWriteFailureExceptionCaptured)
            {
                SuperController.LogError(
                    "Hud [key K]: failed to write request JSON: " + requestWriteFailureExceptionCaptured.Message);
                return;
            }
            ...
            SuperController.LogMessage("Hud [key K]: patch request JSON saved to " + RequestJsonRelative);
            SuperController.LogMessage(
                string.Format(
                    "Hud [key K]: open load folder snapshot — {0} (distinct non-meta scene .json immediate children={1})",
                    loadDirectoryFwdTrimmedNormalized,
            ...
            SuperController.LogMessage("Hud [key K]: auto patch target scene json=" + singleSceneRelativePathChosenFwdNormalized);
            ...
            {
                SuperController.LogError(
                    "Hud [key K]: could not start Python (tried: " + pythonLaunchAttemptsSummaryWide + "). Install Python 3 or add it to PATH.");
            ...
            if (!finishedWaitingUpToDeadlineCaptureWideLate)
            {
                ...
                SuperController.LogError("Hud [key K]: python patch timed out (180s). See " + PatchToolLogRelative);
                return;
            }
            ...
            {
                SuperController.LogError(string.Format(
                    "Hud [key K]: python exit {0}. Details: {1}",
            ...
            else
            {
                SuperController.LogMessage(string.Format(
                    "Hud [key K]: python exit 0. Details: {0}",
                    PatchToolLogRelative));
                SuperController.LogMessage(
                    "Hud [key K]: patch removed root playerNavCollider if present — " +
            ...
            JSONClass serializedRequestCaptured)
        {
            ...
            SuperController.LogError(
                string.Format(
                    "Hud [key K]: automatic patch aborted — {0}. Use the CLI below with YOUR chosen scene .json.",
            ...
            SuperController.LogError(
                string.Format(
                    "Hud [key K]: manual CLI (pick scene path yourself): python \"{0}\" \"ABS_PATH_SCENE.json\" \"{1}\"",
            ...
            SuperController.LogError(
                "Hud [key K]: request payload JSON (paste into file or compare): " +
                requestCompactOneLineCaptured);
        }
        ...
        {
            SuperController.LogMessage("Hud [key K]: folder classification — " + classificationLineCapturedCaptured);
            if (chosenSceneRelativeFwdOrEmptyCaptured.Length > 0)
            {
                SuperController.LogMessage(
                    string.Format(
                        "Hud [key K]: auto-selected exclusive scene JSON under currentLoadDir → {0}",
                        chosenSceneRelativeFwdOrEmptyCaptured));
            }

            EmitPoseSnapshotLinesFromControllers(sceneControllerCaptured);
            SuperController.LogMessage("Hud [key K]: playerHeightAdjust=" + sceneControllerCaptured.playerHeightAdjust.ToString("G9"));
        }
        ...
        private static void EmitPoseSnapshotLinesFromControllers(SuperController sc)
        {
            if (sc.MonitorCenterCamera != null)
            {
                ...
                SuperController.LogMessage(string.Format(
                    "Hud [key K]: monitorCameraRotation (local euler)=({0:F4},{1:F4},{2:F4})",
            ...
            {
                SuperController.LogMessage("Hud [key K]: MonitorCenterCamera is null");
            }
            ...
            if (sc.navigationRig != null)
            {
                ...
                SuperController.LogMessage(string.Format(
                    "Hud [key K]: [CameraRig]/navigationRig world pos=({0:F4},{1:F4},{2:F4}) euler=({3:F4},{4:F4},{5:F4})",
            ...
            {
                SuperController.LogMessage("Hud [key K]: navigationRig is null");
            }
            ...
            if (sc.navigationRig != null && sc.centerCameraTarget != null)
            {
                ...
                SuperController.LogMessage(string.Format(
                    "Hud [key K]: centerCameraTarget world euler=({0:F4},{1:F4},{2:F4})",
                    ceCapturedWide.rotation.eulerAngles.x,
                    ceCapturedWide.rotation.eulerAngles.y,
                    ceCapturedWide.rotation.eulerAngles.z));
                SuperController.LogMessage(string.Format(
                    "Hud [key K]: cameraRig PATCH euler (center-eye, Z=0)=({0:F4},{1:F4},{2:F4})",
                ...
                windowCameraAtomCapturedWide.mainController.control != null)
            {
                ...
                SuperController.LogMessage(string.Format(
                    "Hud [key K]: WindowCamera control world pos=({0:F4},{1:F4},{2:F4}) euler=({3:F4},{4:F4},{5:F4})",
            ...
            {
                SuperController.LogMessage(
                    "Hud [key K]: WindowCamera / mainController / control missing — JSON patch skips WindowCamera when absent.");
            }
        }
