...
//            "Flirting",
        ...
        protected void UpdateRandomParams()
        {
            ...
            morphControl.GetMorphDisplayNames().ForEach((name) =>
            {
                ...
                if (toggles[name].toggle.isOn && morph.animatable == false)
                {
        ...
        public override void Init() {
			try {
                pluginLabelJSON.val = "Expressions";
                UpdateInitialMorphs();
                UpdateNewMorphs();
                UpdateCurrentMorphs();
                ...
                {
                    JSONStorableBool checkBoxTick = new JSONStorableBool(entry.Value, defaultOn.Any((str) => entry.Key.Equals(str)));
