...
public class RecalculateTangents
{
	...
	{
		recalculateTangentsAccurate(_triangles, _vertices, _normals, _uv, ref _tan1, ref _tan2, _tangents);
	}
	...
	public static void recalculateTangentsFast(int[] triangles, Vector3[] vertices, Vector3[] normals, Vector2[] uv, Vector4[] tangents, bool[] vertexDirty = null, bool[] markerArray = null, bool useSleep = false)
	{
		...
		for (int k = 0; k < num2; k++)
		{
			...
			if (markerArray[k])
			{
				float num25 = normals[k].x * tangents[k].x + normals[k].y * tangents[k].y + normals[k].z * tangents[k].z;
				vector.x = tangents[k].x - normals[k].x * num25;
				vector.y = tangents[k].y - normals[k].y * num25;
				vector.z = tangents[k].z - normals[k].z * num25;
				float num26 = Mathf.Sqrt(vector.x * vector.x + vector.y * vector.y + vector.z * vector.z);
				float num27 = 1f / num26;
				tangents[k].x = vector.x * num27;
				tangents[k].y = vector.y * num27;
				tangents[k].z = vector.z * num27;
				tangents[k].w = -1f;
				markerArray[k] = false;
				if (vertexDirty != null)
				{
