...
namespace geesp0t
{
    public class EasyMoanCycleForce : MVRScript
    {
        ...
        protected String cycleForceNamePrefix = "CycleForce_Spank_";
        ...
        public override void Init()
        {
            try
            {
                ...
                cycleForceQuicknessMin = new JSONStorableFloat("Force Quickness Min", 0f, 0f, 10f, false);
                ...
                cycleForceQuicknessMax = new JSONStorableFloat("Force Quickness Max", 6f, 0f, 10f, false);
                ...
                cycleForcePeriodMin = new JSONStorableFloat("Force Period Min", 2f, 0.1f, 10f, false);
                ...
                cycleForceFactorMin = new JSONStorableFloat("Force Factor Min", 0f, 0f, 500f, false);
                ...
                cycleForceFactorMax = new JSONStorableFloat("Force Factor Max", 700f, 0f, 1000f, false);
                ...
                cycleForceRatio = new JSONStorableBool("Force Ratio Linked to Arousal", false);
                ...
                cycleForceRatioMax = new JSONStorableFloat("Force Ratio Max", 0.25f, 0f, 1.0f, false);
        ...
        public void Update()
        {
            try
            {
                explanationString.val = string.Format("Arousal: {0:P}", arousal);

                if (_targetCycleForce != null)
                {
                    if (cycleForceQuickness.val) _targetCycleForce.forceQuickness = Mathf.Lerp(cycleForceQuicknessMin.val, cycleForceQuicknessMax.val, arousal * arousal);
                    if (cycleForcePeriod.val) _targetCycleForce.period = Mathf.Lerp(cycleForcePeriodMin.val, cycleForcePeriodMax.val, Mathf.Sqrt(arousal));
                    if (cycleForceFactor.val) _targetCycleForce.forceFactor = Mathf.Lerp(cycleForceFactorMin.val, cycleForceFactorMax.val, arousal * arousal);
                    if (cycleForceRatio.val) _targetCycleForce.periodRatio = Mathf.Lerp(cycleForceRatioMin.val, cycleForceRatioMax.val, arousal);
                }
            }
            catch (Exception e) { SuperController.LogError("Exception caught: " + e); }
        }
