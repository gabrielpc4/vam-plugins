...
namespace geesp0t
{
    ...
    /// <see cref="GabrielHudButtons.Init"/>, and call it from a private module in
    /// <see cref="VrGestureRuntime"/>.
    /// </summary>
    public sealed class VrGestureBindings
    {
        /// <summary>
    ...
    /// <summary>
    /// Called from <see cref="GabrielHudButtons.ProcessHotkeysUpdate"/> after
    /// keyboard handling. Runs gesture modules in order; a module does nothing
    /// if its binding is null. Built via <c>GabrielHud.cslist</c>.
    /// </summary>
    public static class VrGestureRuntime
    {
        /// <summary>
        ...
        public static void ProcessUpdate(VrGestureBindings bindings)
        {
            if (bindings == null)
                return;
        ...
        private static class DualHandHmdRelativeEulerPossessClosestFemale
        {
            ...
            public static void ProcessUpdate(Action boundAction)
            {
                ...
                Transform camTf = VrEulerPossessPoseCheck.ResolveHmdTransform(sc);
                if (camTf == null)
                    return;
                ...
                bool leftOk = lh != null &&
                    VrEulerPossessPoseCheck.TryHmdRelativeEuler360(
                        lh,
                        camTf,
                        out leftEuler) &&
                    VrEulerPossessPoseCheck.LeftMatches(leftEuler);
                bool rightOk = rh != null &&
                    VrEulerPossessPoseCheck.TryHmdRelativeEuler360(
                        rh,
                        camTf,
                        out rightEuler) &&
                    VrEulerPossessPoseCheck.RightMatches(rightEuler);
                bool poseOk = leftOk && rightOk;

                if (GripHandVisibility.IsAnyPersonHeadOrHandPossessed())
                {
