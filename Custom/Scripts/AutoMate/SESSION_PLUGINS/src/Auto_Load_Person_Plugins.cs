...
namespace geesp0t
{
    public class Auto_Load_Person_Plugins : MVRScript
    {
        ...
        void SetDefaultSettings()
        {
            ...
            femaleSoloPlugins.Add(PLUGIN_E_MOTION);
        ...
        public override void Init()
        {
            ...
            explanationString = new JSONStorableString("", "Each time a scene is loaded, or a Person atom is added, standard plugins are added to all Person atoms.\n\n" +
                "You can use the two buttons to the right of the VAM Menu to change which plugins are loaded for Male and Female people.\n\n" +
                "The settings you create here are only loaded if top button to the right of the VAM Menu shows Custom SETTINGS.\n\n" +
            ...
            {
                mainUIButtons.createdMale = true;
            }
        }
        ...
        public void ShowUI()
        {
            if (mainUIButtons != null)
            {
                ...
                mainUIButtons.autoLoadButton.gameObject.SetActive(showAutoLoadButton == 1);
            }
        }
        public void HideUI()
        {
        ...
        public void SavePrefs(bool resetMorphs = false)
        {
            ...
            if (mainUIButtons != null)
            {
                mainUIButtons.autoLoadType = autoLoadType;
                if (mainUIButtons.autoLoadButton != null) mainUIButtons.autoLoadButton.gameObject.SetActive(showAutoLoadButton == 1);
                mainUIButtons.SetButtonText();
            }
        ...
        public void SetCurrentPluginSet(bool loadNow = false)
        {
            ...
            if (mainUIButtons != null)
            {
                mainUIButtons.cycleSetText = pluginSets[currentPluginSet].buttonName + " >";
                mainUIButtons.cycleSet2Text = pluginSets2[currentPluginSet2].buttonName + " >";
                mainUIButtons.SetButtonText();
            }
        }
        ...
        public void Update()
        {
            if (mainUIButtons != null)
            {
                if (mainUIButtons.wantsToCycleSet)
                {
                    mainUIButtons.wantsToCycleSet = false;
                    NextPluginSet();
                }
                if (mainUIButtons.wantsToCycleSet2)
                {
                    mainUIButtons.wantsToCycleSet2 = false;
                    NextPluginSet2();
                }
                if (mainUIButtons.wantsToCreateMaleAtom)
                {
                    mainUIButtons.wantsToCreateMaleAtom = false;
                    CreateMaleIfNeeded();
                }
                if (mainUIButtons.wantsToRemoveMaleAtom)
                {
                    mainUIButtons.wantsToRemoveMaleAtom = false;
                    RemoveMaleAtom();
                }
                if (mainUIButtons.wantsToStartLicking)
                {
                    mainUIButtons.wantsToStartLicking = false;
                    useImprovedPOVLicking = true;
                    StartLicking();
                }
                if (mainUIButtons.wantsToStopLicking)
                {
                    mainUIButtons.wantsToStopLicking = false;
                    useImprovedPOVLicking = true;
                    StopLicking();
                }
                if (mainUIButtons.wantToChangeAutoLoadType)
                {
                    mainUIButtons.wantToChangeAutoLoadType = false;
                    autoLoadType++;
                    if (autoLoadType >= AUTO_LOAD_COUNT) autoLoadType = 0;
                    SavePrefs(true); //reset morphs
                }
                if (mainUIButtons.wantToLoadNow)
                {
                    mainUIButtons.wantToLoadNow = false;
                    SetCurrentPluginSet(true);
                }
            ...
            if (sceneChanged)
            {
                Log("Scene Changed or loaded this plugin");
                sceneChanged = false;
                appliedPersonPlugins = false;
            ...
            if (sceneChanged && autoLoadType != AUTO_LOAD_DISABLED && !appliedPersonPlugins && !SuperController.singleton.isLoading)
            {
                //wait a little, otherwise it reloads plugins multiple times during loading
                if (!wantToSetAppearance) {
                    appliedPersonPlugins = true;
                    Log("load person plugins from Update");
                    LoadPersonPlugins();
                }
            }
        ...
        void FindMaleAtomLicking(bool adjustUIButtons)
        {
            ...
            if (impPOVTongue != null)
            {
                ...
                useImprovedPOVLicking = true;
                if (adjustUIButtons && mainUIButtons != null)
                {
                    mainUIButtons.createdMale = true;
                    mainUIButtons.isLicking = true;
                }
            }
            else if (impPOV != null)
            {
            ...
            } else if (maleAtomLicking != null)
            {
                ...
                if (adjustUIButtons && mainUIButtons != null)
                {
                    mainUIButtons.createdMale = false;
                    mainUIButtons.isLicking = false;
                }
            } else
            {
        ...
        public bool FileExists(string relativePath)
        {
            ...
            if (pathFileList != null && pathFileList.Length > 0)
            {
                foreach (string foundPathFile in pathFileList)
                {
                    ...
                    //Log("Checking: " + correctedPathFile + " ends with: " + "/" + pathFile);
        ...
        void LoadPersonPlugins(bool maleOnly = false)
        {
            ...
            foreach (Atom at in personAtoms)
            {
                ...
                else
                {
                    ...
                    Log("Num Person Atoms: " + personAtoms.Count<Atom>());

                    if (personAtoms.Count<Atom>() == 1)
                    {
                    ...
                    current["plugins"]["plugin#0"].Value != "")
                {
                    ...
                    foreach (JSONNode pluginNode in current["plugins"].Childs)
                    {
                        ...
                        newPluginFilenames.Add(fileName);
                        Log(at.name + " Already Has Plugin: " + fileName);
                    }
