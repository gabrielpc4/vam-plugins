...
19-08-26 Doc.Oc' registered the chooser so that the gaze (to target or to player) can be modified through a Trigger
*/
using UnityEngine;
...
namespace IdleHelper
{
    public class Gaze : MVRScript
    {
        ...
        try {
				...
				pluginLabelJSON.val = "Head Movement";
				person = containingAtom;

                // Disable auto expression and make sure eyes are set to target
                person.GetStorableByID("AutoExpressions").SetBoolParamValue("enabled", false);
                person.GetStorableByID("Eyes").SetStringChooserParamValue("lookMode", "Target");
                ...
                activationDistance = new JSONStorableFloat("Activation Range (ignore eye saccades)", 0.2f, 0f, 10f, true, true);
				...
				rollAngleMax = new JSONStorableFloat("Roll Angle Maximum", 25f, 0f, 40f, true, true);
                ...
                // Storage:
                focusChangeDurationMax.storeType = JSONStorableParam.StoreType.Full;
        ...
        protected void FixedUpdate()
        {
            ...
            // Randomize offset every few seconds
            offsetTimer += Time.fixedDeltaTime;
            if (offsetTimer > UnityEngine.Random.Range(4,10))
            {
                offsetV.val = UnityEngine.Random.Range(-20, 20);
                offsetH.val = UnityEngine.Random.Range(-20, 20);
                offsetTimer = 0;
            }
            ...
            // only update lookAtPosition if it has changed significantly (so gaze doesn't follow eye sacades)
//            if (Vector3.Distance(lookAtPosition, gazeTarget.TransformPoint(gazeOffset)) > activationDistance.val)
//            {
////                SuperController.LogMessage("Updating lookatPosition");
//                lookAtPosition = gazeTarget.TransformPoint(gazeOffset);
//            }
