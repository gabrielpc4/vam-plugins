// Gaze v1.2 by MacGruber. Adding roll min/max and horizontal eye angle offset.
// 
// History:
    ...
    {	
        public override void Init()
		{
			try 
			{
				...
				rollAngleMin = SetupSlider("Roll Angle Min", -6f, -25f, 25f, false);
				rollAngleMax = SetupSlider("Roll Angle Max", 6f, -25f, 25f, false);
				
				eyeAngleVOffset = SetupSlider("Eye Angle Vertical Offset", -5.0f, -20.0f, 20.0f, false);
				eyeAngleHOffset = SetupSlider("Eye Angle Horizontal Offset", 0.0f, -20.0f, 20.0f, false);
				eyePositionOffset = SetupSlider("Eye Position Offset", 0.1f, -0.2f, 0.2f, true);
				lookAtPositionOffset = SetupSlider("LookAt Position Offset", 0.0f, -0.2f, 0.2f, true);
        ...
        {		
			...
			Vector3 lookAtPosition = lookAtTarget.TransformPoint(lookAtPositionOffset.val * Vector3.up);
			...
			targetH += eyeAngleHOffset.val * Mathf.Deg2Rad;
			targetV += eyeAngleVOffset.val * Mathf.Deg2Rad;
            ...
            if (rollChangeClock >= 1.0f)
            {
                ...
                rollPrev = rollNext;
				float rmin = Mathf.Min(rollAngleMin.val, rollAngleMax.val);
				float rmax = Mathf.Max(rollAngleMin.val, rollAngleMax.val);
                rollNext = Random.Range(rmin, rmax) * Mathf.Deg2Rad;
            }
		...
		private JSONStorableFloat rollAngleMin;
		private JSONStorableFloat rollAngleMax;		
		private JSONStorableFloat eyePositionOffset;
		private JSONStorableFloat eyeAngleVOffset;
		private JSONStorableFloat lookAtPositionOffset;
		private JSONStorableFloat eyeAngleHOffset;
