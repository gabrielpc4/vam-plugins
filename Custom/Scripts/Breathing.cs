	...
	public class B : MVRScript {
        ...
        protected void Update()
        {
            ...
            if (toggles["Play"].toggle.isOn)
            {
                ...
                {
                    morphMouth.morphValue = (-1*varia -1) * animLengthSlider.val * mouthLinkSlider.val/25 + ((mouthLinkSlider.val/2 * (animLengthSlider.val - 2)) + (intensitySlider.val-0.7f)/2f + (animLengthSlider.val-2) / 24)*intensitySlider.val;
                }
        ...
        public override void Init() {
            try
            {
                ...
                intensitySlider = new JSONStorableFloat("Intensity", intensityDefaultValue, 0f, 1.0f, true);
                intensitySlider.storeType = JSONStorableParam.StoreType.Full;
                RegisterFloat(intensitySlider);
                CreateSlider(intensitySlider, true);

                animLengthSlider = new JSONStorableFloat("Speed", speedDefaultValue, 0.1f, 10f, true);
                ...
                chestSlider = new JSONStorableFloat("Chest rotation intensity", chestRotationDefaultValue, 0f, 2f, true);
                chestSlider.storeType = JSONStorableParam.StoreType.Full;
                RegisterFloat(chestSlider);
                CreateSlider(chestSlider, true);

                mouthLinkSlider = new JSONStorableFloat("Mouth link intensity", mouthLinkDefaultValue, 0f, 2f, true);
                mouthLinkSlider.storeType = JSONStorableParam.StoreType.Full;
                RegisterFloat(mouthLinkSlider);
                CreateSlider(mouthLinkSlider, true);

                ribcageSlider = new JSONStorableFloat("Ribcage size link intensity", ribcageSizeLinkDefaultValue, -2f, 2f, true);
                ribcageSlider.storeType = JSONStorableParam.StoreType.Full;
                RegisterFloat(ribcageSlider);
                CreateSlider(ribcageSlider, true);

                sternumSlider = new JSONStorableFloat("Sternum link intensity", sternumLinkDefaultValue, -2f, 2f, true);
        ...
        private float intensityDefaultValue       = 0.7f;
        private float speedDefaultValue           = 1.0f;
        private float chestRotationDefaultValue   = 1.0f;
        private float mouthLinkDefaultValue       = 1.0f;
