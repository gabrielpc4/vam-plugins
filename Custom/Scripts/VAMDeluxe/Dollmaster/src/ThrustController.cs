...
namespace DeluxePlugin.Dollmaster
{
    public class ThrustController : BaseModule
    {
        ...
        public void NotifyVAMLaunch() {
            Atom _scriptConnectionAtom = null; //connect to VAMLaunch
            Transform _scriptConnectionTransform = null;
            if (_scriptConnectionAtom == null)
            {
                _scriptConnectionAtom = SuperController.singleton.GetAtomByUid("VAMLaunchScriptConnection");
                if (_scriptConnectionAtom != null)
                {
                    _scriptConnectionTransform = _scriptConnectionAtom.transform;
                    if (_scriptConnectionTransform != null)
                    {
                        _scriptConnectionTransform.position = new Vector3(_scriptConnectionTransform.position.x, _scriptConnectionTransform.position.y, 0.2f); //0.2 means switch to dollmaster
                    }
                }
            }
        }
