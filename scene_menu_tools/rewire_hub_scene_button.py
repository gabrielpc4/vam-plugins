...
Examples:
  python scene_menu_tools/rewire_hub_scene_button.py --button-id TopLeftB7 \\
  ...
  python scene_menu_tools/rewire_hub_scene_button.py --button-id RightTopB6 \\
...
def scene_type_for_button_id(button_id: str) -> str | None:
...
def build_button_label(scene_title: str, button_id: str, author: str | None) -> str:
    lines = [scene_title.strip(), ""]
    typ = scene_type_for_button_id(button_id)
    if typ:
        lines.append(f"Type: {typ}")
    if author and author.strip():
        if author.strip().lower() != scene_title.strip().lower():
...
def main() -> int:
    ...
    label = build_button_label(scene_title, bid, author)
