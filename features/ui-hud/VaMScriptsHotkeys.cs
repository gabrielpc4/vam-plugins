...
namespace geesp0t
{
    ...
    internal sealed class Hotkeys
    {
        ...
        private const string SessionOrchestratorSuffix =
            ".SessionOrchestrator";
        private const string ForceReleaseSceneSettleHoldActionName =
            "ForceReleaseSceneSettleHold";

        private readonly Hud owner;

        internal Hotkeys(Hud owner)
        {
            this.owner = owner;
        }
        ...
        internal void ProcessUpdate()
        {
            ...
            if (Input.GetKeyDown(KeyCode.F))
            {
                try
                {
                    Hud.ToggleFreezeAnimationHotkey();
                }
                catch (Exception e)
                {
                    SuperController.LogError(
        ...
        private static void TryForceReleaseSceneSettleHoldFromHotkey()
        {
            ...
            sessionPluginsStorable =
                FindSessionPluginStorableBySuffix(
                    coreControl,
                    SessionOrchestratorSuffix);
            if (sessionPluginsStorable == null)
            {
                SuperController.LogError(
                    "Space hotkey: SessionOrchestrator plugin not " +
            ...
            {
                SuperController.LogError(
                    "Space hotkey: SessionOrchestrator action <" +
                    ForceReleaseSceneSettleHoldActionName +
                    "> is unavailable.");
                return;
            }
