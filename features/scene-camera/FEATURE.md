...
- Compiled in **`SessionPlugins.cslist`** (mirrored by
  `Hud.cslist`).
- `SceneCameraPatch` writes request data into `Custom/Scripts/tools/scene-camera/` and can auto-run the Python patcher.
...
- Depends on `Custom/Scripts/tools/scene-camera/FEATURE.md` for the offline script side.

## References
- `Custom/Scripts/FEATURE.md`
- `Custom/Scripts/tools/scene-camera/FEATURE.md`
