...
namespace VRAdultFun
{
    partial class EmotionEngine : MVRScript
    {
		...
		/// <summary>True when this atom is the only <c>Person</c> in the scene, VR hand transforms exist, and we are not tracking another Person as person2 — desktop/HMD + single character, where gaze should sometimes follow hands instead of only the camera.</summary>
		private static bool SoloVrSceneSinglePersonCharacter()
		{
...
/*					if (currentAtom.type == "Person")
        ...
        public void FixedUpdate()
        {
            ...
            if (playerHandsUsable || person2Usable)
            {
                ...
                if (SoloVrSceneSinglePersonCharacter())
                {
                    interestLHand += 0.03f * uiInterestRate.val;
                    interestRHand += 0.03f * uiInterestRate.val;
                }
            ...
            if (interestClock <= 0.0f && amGlancing == false)
            {
				...
				*/
				...
				if (interestRepeating == false)
				{
					...
					secondValue = 0.0f;
					tempFloat = 0.0f;
					bool soloVrOnePerson = SoloVrSceneSinglePersonCharacter();
					float handMainMinInterest = soloVrOnePerson ? 12.0f : 23.0f;
					float handVsFaceMargin = soloVrOnePerson ? -30.0f : -10.0f;
					...
					if (Mathf.Abs(Vector3.Angle(playerLHandTransform.position - headController.followWhenOff.position, headController.followWhenOff.forward)) < lookPeripheralAngle || interestLHand + lHandActivityBoost > interestFace + headActivityBoost || soloVrOnePerson)
					{
						if (interestLHand + lHandActivityBoost > secondValue && (playerHandsUsable || (person2Usable && usePerson2)))
						{
							...
							{
								if (interestLHand + lHandActivityBoost > handMainMinInterest && interestLHand + lHandActivityBoost > interestFace + handVsFaceMargin)
								{
					...
					if (Mathf.Abs(Vector3.Angle(playerRHandTransform.position - headController.followWhenOff.position, headController.followWhenOff.forward)) < lookPeripheralAngle || interestRHand + rHandActivityBoost > interestFace + headActivityBoost || soloVrOnePerson)
					{
						if (interestRHand + rHandActivityBoost > secondValue && (playerHandsUsable || (person2Usable && usePerson2)))
						{
							...
							{
								if (interestRHand + rHandActivityBoost > handMainMinInterest && interestRHand + rHandActivityBoost > interestFace + handVsFaceMargin)
								{
