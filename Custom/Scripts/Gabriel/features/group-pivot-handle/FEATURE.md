...
spawned Cube pivot (`Atom.parentAtom`). **Not part of Gabriel session bootstrap
or `GabrielSessionPlugins.cslist`**.
...
Merge **`Custom/Scripts/Gabriel/features/group-pivot-handle/MultiAtomPivotHandle.cslist`**
onto **CoreControl** (or any atom) only when you want the tool. Do **not** add
this path to `GabrielSessionPlugins.cslist` unless you intend it to load every
session.
...
- [`Custom/Scripts/Gabriel/FEATURE.md`](../FEATURE.md)
