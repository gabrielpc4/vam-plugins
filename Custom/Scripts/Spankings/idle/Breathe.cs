...
namespace IdleHelper
{
    public class Breathe : MVRScript
    {
		...
		public static float breatheCycle = 0.0f;
		public static float breatheDuration = 1.0f;
		...
		public static JSONStorableFloat intensity;
		private JSONStorableFloat variance;
		public static JSONStorableFloat speedAdjust;
		...
		private JSONStorableFloat pitchRange;
		private JSONStorableBool  allowMoan;
		private JSONStorableBool  disableSound;
		...
		{
			intensity = SetupSlider("Intensity", 0.2f, 0.0f, 1.0f, false);
			variance = SetupSlider("Variance", 0.2f, 0.0f, 0.5f, true);			
			speedAdjust = SetupSlider("SpeedAdjust", 0.0f, 0.0f, 10.0f, false);		
			...
			mouthOpenMax = SetupSlider("MouthOpenMax", 0.2f, -0.5f, 2.0f, true);
			pitchRange = SetupSlider("Pitch Range", 0.05f, 0.0f, 1.0f, false);
		    CreateSpacer(true);
			allowMoan = SetupToggle("AllowMoan", false, false);
			disableSound = SetupToggle("Disable Sound", false, false);
        ...
        {		
			...
			if (breatheCycle >= breatheDuration)
			{
			    ...
			    {
                    // randomize pitch
                    pitchCurrent = pitchBase + UnityEngine.Random.Range(-pitchRange.val, pitchRange.val);
