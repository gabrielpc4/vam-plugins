...
namespace geesp0t
{
    ...
    public class Hud : MVRScript
    {
        ...
        private const string SessionOrchestratorClassSuffix =
            ".SessionOrchestrator";
        ...
        private void OnDestroy()
        {
            ...
            catch (Exception e)
            {
                SuperController.LogError(
                    "Hud.OnDestroy orchestrator bind: " + e.Message);
            }
            ...
            {
                SuperController.LogError(
                    "Hud.OnDestroy teardown: " + e.Message);
            }
        }
        ...
        private void TryBindSessionOrchestrator()
        {
            ...
            for (i = 0; i < storableIds.Count; i++)
            {
                ...
                if (string.IsNullOrEmpty(storableId) ||
                    !storableId.EndsWith(
                        SessionOrchestratorClassSuffix,
                ...
                orchestrator = storable as SessionOrchestrator;
                if (orchestrator == null)
                    continue;

                _orchestrator = orchestrator;
                _orchestrator.BindHud(this);
                return;
            }
        }
            ...
            int row)
        {
            ...
            button = CreateButton(
                ...
                HudButtonWidth,
                HudButtonHeight,
                HudButtonLabelFontSize);

            translateX = column * HudGridXSpacing;
            translateY = HudGridOriginY -
                row * HudGridYSpacing;
