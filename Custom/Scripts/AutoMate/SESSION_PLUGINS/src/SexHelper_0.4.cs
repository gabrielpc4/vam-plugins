...
namespace geesp0t
{
    public class SexHelper
    {
        ...
        private BreathEntry[] breathEntries = null;
        ...
        Atom containingAtom = null;

        public SexHelper(Atom atom)
        {
            containingAtom = atom;
            ready = false;
            try
            {
                ...
                autoStart = new JSONStorableBool("Start on Scene Load", false);
                ...
                penisEnabledJSON = new JSONStorableBool("Penis Thrust Enabled", false);
                ...
                */
                ...
                hipEnabledJSON = new JSONStorableBool("Hip Thrust Enabled (Experimental)", true);
                ...
                grindMultiplierJSON = new JSONStorableFloat("Grind Speed Multiplier (lower = faster)", 2f, 0f, 3f, false);
        ...
        public void Update()
        {
            ...
            {                
                if (!ready && !SuperController.singleton.isLoading)
                {
                    ...
                    //SuperController.LogMessage("Bla");
                    ...
                    //SuperController.LogMessage("Ble");
                    foreach (Atom at in personAtoms)
                    {
                        if (at.GetComponentInChildren<DAZCharacter>().isMale) {
                            him = at;
                            //SuperController.LogMessage("him = " + him.uid);
                            break;
                        }
                    }

                    //SuperController.LogMessage("Bli");

                    if (him == null) {
                        //SuperController.LogMessage("No him");
                        ...
                        //SuperController.LogMessage("Yes him");
                        ...
                        //SuperController.LogMessage("Blu");
                    }

                    SyncFemaleAtom();

                    //SuperController.LogMessage("BlAA");

                    if (autoStart.val)
                    {
                        //SuperController.LogMessage("Blee");
                        StartSex();
                    }
                    
                    ready = true;
                    //SuperController.LogMessage("Blii");
                }
        ...
        public void FixedUpdate()
        {
            ...
            try
            {
                ...
                if (isActive && containingAtom != null)
                {
                    ...
                    */
                    ...
                    {
                        //SuperController.LogMessage("13");
                        perc = lerpTimer / lerpTime;
                        perc = easing(perc);
                        hipCurrent = Vector3.Lerp(hipStart, hipTarget, perc);
                        hipControl.transform.position = hipCurrent;
                        //SuperController.LogMessage("14");
                    }
                    //SuperController.LogMessage("15");
                    if (hipCurrent == hipTarget && hipEnabledJSON.val && !alignOnlyDisableMotion.val)
                    {
                        //SuperController.LogMessage("16");
                        NewThrust("hip");
                        //SuperController.LogMessage("17");
                    }

                    // update grind
                    //SuperController.LogMessage("18");
                    ...
                    //SuperController.LogMessage("19");
                    if (grindCurrent == grindTarget)
                    {
                        //SuperController.LogMessage("20");
                        SetNewGrindTarget();
                    }
                    //SuperController.LogMessage("21");
...
//            grindTarget.x = UnityEngine.Random.Range(-grindJSON.val, grindJSON.val);
        ...
        private float SetNewDurationTimer()
        {
            if (durationRangeJSON.val == 0 || durationUpdateTimer > durationUpdateIntervalJSON.val) 
            {
                ...
                return UnityEngine.Random.Range(0.2f, max);
            }
            else
            {    
        ...
        {
            SuperController.LogMessage("Start Sex");
            penisPosition = -2.1f;
...
//            SetNewPenisTarget();
           ...
           /* penisBase.currentPositionState = FreeControllerV3.PositionState.On;
            ...
            if (breathEntries == null)
                breathEntries = new BreathEntry[] {
