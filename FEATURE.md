...
## Bootstrap (`bootstrap/Bootstrap.cs`)
...
1. `features/ui-hud/VaMLogClipboardHud.cslist` — copy/clear logs (its **own**
   compile).
2. `session-plugins/SessionPlugins.cslist` — **main session assembly** (see
   below).
...
## Session bundle (`SessionPlugins.cslist`)

Single plugin URL; one compile that typically includes:

- **`session-plugins/src/*`** — `SessionOrchestrator`,
...
- **`features/ui-hud/*`** — `Hud`, `Hotkeys`, `NextSceneUiButton`
...
**Offline parity:** `features/ui-hud/Hud.cslist` lists the **same** `.cs`
...
in `SessionPlugins.cslist` or VaM would also run it on CoreControl.
...
`SessionPlugins.cslist` membership changes in a way users must see at a
